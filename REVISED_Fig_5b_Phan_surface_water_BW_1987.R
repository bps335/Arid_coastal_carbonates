...
############################# Run the model ##############################
  ...
  if (t == t_low){
    assign('phan_low_cal',(10^cal_out))
    assign('phan_low_arag',(10^arag_out))
  } 
  
  else if (t == t_high){
    assign('phan_high_cal',(10^cal_out))
    assign('phan_high_arag',(10^arag_out))  
  }
  ...
  phan_low_cal_out = (3.6*(phan_low_cal-1)^1.9)/(3.6*(phan_low_cal[n]-1)^1.9)
  phan_high_cal_out = (3.7*(phan_high_cal-1)^2.3)/(3.6*(phan_low_cal[n]-1)^1.9)
  
  phan_low_arag_out = (40.6*(phan_low_arag-1)^1.7)/(3.6*(phan_low_cal[n]-1)^1.9)
  phan_high_arag_out = (45.1*(phan_high_arag-1)^2.4)/(3.6*(phan_low_cal[n]-1)^1.9)
  ...
  ggplot(data, aes(x = Age.bin)) +  
    geom_line( y = phan_low_cal_out, size = 1.2, color = 'black') +
    geom_line( y = phan_high_cal_out, size = 1.2, color = 'black') +
    geom_ribbon(aes(x = Age.bin, ymin=phan_low_cal_out, ymax = phan_high_cal_out), 
                fill = 'blue', alpha=0.3) +
    scale_x_reverse() + ylim(0, 50) +
    theme_classic() +
  
  ## Add labels and tidy up the plot for export.
  
    labs (x = 'Age (Ma)', y = 'Precipitation factor', 
          title = 'Calcite surface water model') +
  ...
  ggplot(data, aes(x = Age.bin)) +  
    geom_line( y = phan_low_arag_out, size = 1.2, color = 'black') +
    geom_line( y = phan_high_arag_out, size = 1.2, color = 'black') +
    ...
    labs (x = 'Age (Ma)', y = 'Precipitation factor', 
          title = 'Aragonite surface water model') +
    theme(plot.title = element_text(size = rel(1.5), hjust = .5)) + 
    theme(axis.title  = element_text(size = rel(1.2), hjust = .5)) 
