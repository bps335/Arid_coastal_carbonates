########### Sensitivity Analysis for porewater-equilibrium model ############### 

  ## This script performs a sensitivity analysis on the porewater-equilibirum 
  ## model and re-creates Figure 3b.  
...
######################### Run the calcite model ##############################
  ...
  cal_out[bin] <- tail(calcite, n = 1)
  
  ## Close the age loop for a single row in the input file
...
######################### Run the aragonite model ##############################
  ...
  for (row_2 in 1:n)
  {
    ...
    arag_out[bin] <- tail(aragonite, n = 1)
...
#################### Collect and plot data #####################################  
  ...
  cal_norm = cal_out/cal_out[1]
  cal_norm[n] <- tail(arag_out, n =1)/cal_out[1]
