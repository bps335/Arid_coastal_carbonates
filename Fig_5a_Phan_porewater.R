...
###################### Set model parameters ##############################
  ...
  if (t == t_low){
    assign('phan_low',(cal_out))
  } 
  
  else if (t == t_mid){
    assign('phan_mid',(cal_out))  
  }
  else if (t == t_high){
    assign('phan_high',(cal_out))  
  }
  ...
  phan_low_out = (phan_low)/(phan_mid[n])
  phan_mid_out = (phan_mid)/(phan_mid[n])
  phan_high_out = (phan_high)/(phan_mid[n])
  ...
  ## Plot the upper and lower temperature bounds and fill the area inbetween
  ## as an estimate of the uncertainty. 
    
  geom_ribbon(aes(ymin=phan_low_out, ymax = phan_high_out), fill = 'blue', 
                 alpha=0.3) +
  ...
  labs (x = 'Age (Ma)', y = 'Precipitation factor', title = 'Porewater model') +
